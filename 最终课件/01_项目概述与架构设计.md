# 项目概述及骨架代码开发

## 1. 项目概述

### 1.1 项目概述

本项目是一个教学版深度研究报告平台。用户输入研究主题、研究目标、目标读者、地域范围和时间范围后，系统会围绕这个研究问题生成研究任务书和研究大纲；用户确认大纲后，系统再执行资料检索、事实整理、逐章节写作和报告渲染，最终产出一份可追溯的 HTML 研究报告。

### 1.2 和ChatBot（聊天机器人）的区别

这个系统不是普通的 chatbot。普通 chatbot 主要围绕“对话”展开：用户问一句，模型答一句，系统重点维护的是上下文消息。

深度研究报告平台主要围绕“研究项目”展开：系统要维护项目状态、任务状态、大纲、章节、来源和报告版本。它不是只给出一次回答，而是把一个模糊研究问题推进成一个可核查、可保存、可继续扩展的研究产物。

两类系统的区别如下：

| 对比项   | 普通 chatbot  | 深度研究报告平台               |
| ----- | ----------- | ---------------------- |
| 核心对象  | 对话消息        | 研究项目                   |
| 用户输入  | 即时问题        | 研究主题和研究设定              |
| 系统过程  | 直接生成回答      | 生成大纲、确认大纲、执行研究、生成报告    |
| 状态管理  | 对话历史        | 项目状态、任务状态、报告版本         |
| 信息来源  | 模型知识或临时检索结果 | 外部公开资料、内部知识库           |
| 输出结果  | 一段回答        | HTML 报告、章节正文、关键发现、引用来源 |
| 可靠性要求 | 用户自行判断      | 结论需要能追溯到来源             |

### 1.3 项目主流程

**深度研究** 场景，需要解决的三大核心问题，如下所示：

1. LLM 的能力应该在什么地方使用？

2. 如何让报告中的关键结论能够追溯到来源，避免模型幻觉？

3. 如何实现 Human In The Loop，也就是让用户参与到整个过程当中，指引正确的方向？

基于这些问题，深度研究流程可以拆成如下过程：

```mermaid
flowchart TD

    用户输入[用户输入研究主题和设定]
    研究项目[系统生成研究项目]
    研究任务书[生成研究大纲]
    用户确认大纲[用户确认大纲]
    已确认大纲[已确认大纲]
    研究管理智能体[研究管理智能体]
    信息检索智能体[信息检索智能体]
    外部资料[外部公开资料]
    内部资料[内部知识库资料]
    来源列表[来源列表]
    事实材料[事实材料]
    章节结果[逐章节保存]
    项目来源[维护整个项目级别信息来源]
    渲染输入[基于逐章节内容渲染]
    用户反馈[用户阅读和核查]

    用户输入 --> 研究项目
    研究项目 --> 研究任务书
    研究任务书 --> 用户确认大纲
    用户确认大纲 --> 已确认大纲
    已确认大纲 --> 开始研究
    开始研究 --> 研究管理智能体

    研究管理智能体 --> 信息检索智能体
    信息检索智能体 --> 外部资料
    信息检索智能体 --> 内部资料
    外部资料 --> 来源列表
    内部资料 --> 来源列表
    来源列表 --> 事实材料
    事实材料 --> 章节结果
    来源列表 --> 章节结果
    章节结果 --> 项目来源
    章节结果 --> 渲染输入
    项目来源 --> 渲染输入
    渲染输入 --> 用户反馈
```

各个环节所做的事情：

- 创建项目：系统接收用户提交的研究主题和基础设定，生成研究项目，并保存项目初始状态。

- 生成研究任务书和大纲：研究管理智能体理解研究目标、范围和读者，生成研究任务书和大纲草案。

- 确认大纲：用户确认大纲，或者提交修改意见。只有大纲确认后，系统才进入正式研究阶段。

- 执行研究：**研究管理智能体**根据已确认大纲拆解章节问题，并调用信息检索智能体收集资料。

- 构建证据：信息检索智能体通过外部搜索、网页读取和内部知识库检索获得资料，整理来源、事实材料和冲突信息，返回给研究管理智能体。

- 保存章节：研究管理智能体基于来源和事实材料写出章节正文、关键发现和风险说明，并逐章节保存。

- 生成报告：系统从已保存的 `sections` 和项目级去重 `sources` 临时组装渲染输入，使用确定性报告渲染工具生成 HTML 报告。

- 核查报告：报告保留来源列表，关键事实可以通过引用回溯到来源。

## 2. 骨架开发

### 2.1 系统调用时序图

基于前面的业务流程，可以将系统调用过程拆成几个阶段：

1. 创建研究项目，自动启动研究任务书和大纲生成任务。

2. 查询大纲并确认大纲，确定正式研究边界。

3. 提交报告生成任务，并在任务完成后读取最新报告。

4. 后台执行研究、检索资料并渲染报告。

下面对这4个过程的调用时序图，一一展开介绍。

#### 2.1.1 创建项目并生成大纲

```mermaid
%%{init: {'theme': 'base', 'themeVariables': { 'fontSize': '16px'}}}%%
sequenceDiagram

    participant 用户
    participant 前端工作台
    participant 后端接口服务
    participant 后台任务
    participant 研究管理智能体
    participant 数据库

    用户->>前端工作台: 输入研究主题和基础设定
    前端工作台->>后端接口服务: POST /api/v1/research-projects
    后端接口服务->>数据库: 保存研究项目，状态 brief_generating
    后端接口服务->>数据库: 创建大纲生成任务，状态 queued
    后端接口服务->>后台任务: 投递 Celery 大纲生成任务
    后端接口服务-->>前端工作台: 返回 project_id、initial_task_id 和 next_step

    后台任务->>数据库: 更新任务状态 running
    后台任务->>研究管理智能体: 生成研究任务书和大纲草案
    研究管理智能体->>研究管理智能体: 理解主题、目标、范围和默认假设
    研究管理智能体-->>后台任务: 返回研究任务书和大纲草案
    后台任务->>数据库: 保存研究任务书和大纲草案
    后台任务->>数据库: 更新项目状态 outline_ready，任务状态 succeeded
```

这一阶段的关键点是：接口请求很快返回，真正耗时的大纲生成放到后台任务里执行。

#### 2.1.2 查询并确认大纲

```mermaid
%%{init: {'theme': 'base', 'themeVariables': { 'fontSize': '16px'}}}%%
sequenceDiagram

    participant 用户
    participant 前端工作台
    participant 后端接口服务
    participant 后台任务
    participant 研究管理智能体
    participant 数据库

    前端工作台->>后端接口服务: GET /api/v1/tasks/{task_id}
    后端接口服务->>数据库: 读取任务状态
    后端接口服务-->>前端工作台: 返回任务状态
    前端工作台->>后端接口服务: GET /api/v1/research-projects/{project_id}/outline
    后端接口服务->>数据库: 读取项目状态和大纲草案
    后端接口服务-->>前端工作台: 返回大纲草案

    用户->>前端工作台: 确认大纲或提交修改意见
    前端工作台->>后端接口服务: PUT /api/v1/research-projects/{project_id}/outline
    alt 直接确认
        后端接口服务->>数据库: 保存 confirmed_outline，状态 outline_confirmed
        后端接口服务-->>前端工作台: 返回 next_step generate_report
    else 修改大纲
        后端接口服务->>数据库: 创建大纲修改任务，状态 outline_revising
        后端接口服务->>后台任务: 投递 Celery 大纲修改任务
        后端接口服务-->>前端工作台: 返回 revision_task_id 和 wait_for_outline
        后台任务->>数据库: 更新任务状态 running
        后台任务->>研究管理智能体: 根据修改意见调整大纲
        研究管理智能体-->>后台任务: 返回新的大纲草案
        后台任务->>数据库: 保存新大纲，状态 outline_ready
        后台任务->>数据库: 更新任务状态 succeeded
    end
```

这一阶段的关键点是：用户确认大纲后，系统才进入正式研究；如果用户修改大纲，则重新进入后台修订流程。

#### 2.1.3 提交报告任务并读取结果

```mermaid
%%{init: {'theme': 'base', 'themeVariables': { 'fontSize': '16px'}}}%%
sequenceDiagram

    participant 用户
    participant 前端工作台
    participant 后端接口服务
    participant 后台任务
    participant 数据库

    用户->>前端工作台: 请求生成研究报告
    前端工作台->>后端接口服务: POST /api/v1/research-projects/{project_id}/report-tasks
    后端接口服务->>数据库: 校验项目状态为 outline_confirmed
    后端接口服务->>数据库: 创建报告生成任务，项目状态 research_running
    后端接口服务->>后台任务: 投递 Celery 报告生成任务
    后端接口服务-->>前端工作台: 返回 task_id 和 queued

    前端工作台->>后端接口服务: GET /api/v1/tasks/{task_id}
    后端接口服务->>数据库: 读取任务状态
    后端接口服务-->>前端工作台: 返回 running 或 succeeded

    前端工作台->>后端接口服务: GET /api/v1/research-projects/{project_id}/reports/latest
    后端接口服务->>数据库: 读取最新报告版本
    后端接口服务-->>前端工作台: 返回 HTML 报告、版本号和来源列表
    用户->>前端工作台: 阅读报告并通过引用核查关键事实
```

这一阶段的关键点是：前端只负责提交任务、轮询状态和读取结果，不直接等待长时间研究流程结束。

#### 2.1.4 后台执行研究并渲染报告

```mermaid
%%{init: {'theme': 'base', 'themeVariables': { 'fontSize': '16px'}}}%%
sequenceDiagram

    participant 后台任务
    participant 研究管理智能体
    participant 信息检索智能体
    participant 外部搜索工具
    participant 网页读取工具
    participant 内部知识库工具
    participant 报告渲染工具
    participant 数据库

    后台任务->>数据库: 更新任务状态 running
    后台任务->>研究管理智能体: 根据已确认大纲执行研究
    研究管理智能体->>信息检索智能体: 分派章节问题和检索问题
    信息检索智能体->>外部搜索工具: 检索公开资料
    外部搜索工具-->>信息检索智能体: 返回标题、URL、摘要
    信息检索智能体->>网页读取工具: 读取关键网页正文
    网页读取工具-->>信息检索智能体: 返回正文片段和来源信息
    信息检索智能体->>内部知识库工具: 按需检索内部知识库
    内部知识库工具-->>信息检索智能体: 返回内部资料片段和引用
    信息检索智能体-->>研究管理智能体: 返回来源、事实和证据材料
    研究管理智能体->>研究管理智能体: 整理事实材料、关键发现、风险和章节内容
    研究管理智能体->>数据库: 调用 save_research_section 逐章节保存
    研究管理智能体-->>后台任务: 返回章节保存摘要

    后台任务->>数据库: 读取已保存 sections 和项目级 sources
    后台任务->>报告渲染工具: 调用 write_html_report 渲染 HTML
    报告渲染工具-->>后台任务: 返回 HTML、标题和来源列表
    后台任务->>数据库: 保存报告版本，项目状态 report_ready
    后台任务->>数据库: 更新任务状态 succeeded
```

这条链路中，HTTP 接口只负责接收请求、创建任务、返回状态和读取结果。真正耗时的 LLM 调用、搜索、网页读取、知识库检索和报告渲染，都放在后台任务中执行。

### 2.2 研究项目状态设计

本项目所设计的任务状态，有如下两类：

第一类是研究项目状态，描述整个研究项目走到了哪个业务阶段：

| 项目状态                | 含义                   |
| ------------------- | -------------------- |
| `created`           | 项目已创建                |
| `brief_generating`  | 研究任务书和大纲生成中          |
| `outline_ready`     | 大纲草案已生成，等待用户确认       |
| `outline_revising`  | 用户提交了大纲修改意见，系统正在修改大纲 |
| `outline_confirmed` | 用户已确认大纲              |
| `research_running`  | 研究管理智能体正在协调信息检索和分析   |
| `report_ready`      | 报告已生成                |

第二类是后台任务状态，描述某一个后台任务的执行进度：

项目状态和任务状态要分开。一个项目可以有多个任务，例如：

| 任务类型                      | 触发动作       | 对应项目状态变化                                  |
| ------------------------- | ---------- | ----------------------------------------- |
| `generate_research_brief` | 创建研究项目     | `brief_generating` -> `outline_ready`     |
| `revise_outline`          | 用户要求修改大纲   | `outline_ready` -> `outline_revising`     |
| `generate_report`         | 用户提交报告生成任务 | `outline_confirmed` -> `research_running` |

项目主状态流转如下：

```mermaid
stateDiagram-v2
    [*] --> created
    created --> brief_generating: 创建研究项目
    brief_generating --> outline_ready: 大纲生成成功
    brief_generating --> brief_generating: 大纲生成失败后可重试
    outline_ready --> outline_revising: 用户提交修改意见
    outline_revising --> outline_ready: 大纲修改成功
    outline_ready --> outline_confirmed: 用户确认大纲
    outline_confirmed --> research_running: 提交报告生成任务
    research_running --> report_ready: 研究和报告生成成功
```

后台任务状态流转如下：

```mermaid
stateDiagram-v2
    [*] --> queued
    queued --> running: 后台任务开始执行
    running --> succeeded: 执行成功
    running --> failed: 执行失败
```

### 2.3 后端模块划分

本项目后端可以划分为如下模块：

| 模块           | 作用                                          | 典型文件                               |
| ------------ | ------------------------------------------- | ---------------------------------- |
| `main`       | 创建 FastAPI 应用，注册路由，提供健康检查和静态文件挂载            | `app/main.py`                      |
| `routers`    | 定义 HTTP 接口，处理请求校验、状态判断、任务创建和结果返回            | `app/routers/__init__.py`          |
| `schemas`    | 定义请求结构、响应结构、枚举状态和核心数据模型                     | `app/schemas/__init__.py`          |
| `config`     | 管理系统配置，例如 API 前缀、模型配置、MongoDB 地址、RAGFlow 地址 | `app/config/config.py`             |
| `background` | 管理 Celery 任务投递和 worker 执行流程，执行长耗时研究任务       | `app/background/research_tasks.py` |
| `agents`     | 构建研究管理智能体和信息检索智能体，生成大纲并逐章节保存研究结果            | `app/agents/research_agent.py`     |
| `tools`      | 提供外部搜索、网页读取、RAGFlow 检索和报告渲染等工具能力            | `app/tools/*.py`                   |
| `repository` | 封装 MongoDB 读写，保存项目、任务、研究结果和报告版本             | `app/repository/*.py`              |

模块之间的调用关系如下：

```mermaid
flowchart LR
    前端[前端工作台]
    路由[routers]
    结构[schemas]
    后台[background]
    智能体[agents]
    工具[tools]
    仓储[repository]
    数据库[(MongoDB)]

    前端 --> 路由
    路由 --> 结构
    路由 --> 后台
    路由 --> 仓储
    后台 --> 智能体
    后台 --> 工具
    后台 --> 仓储
    智能体 --> 工具
    仓储 --> 数据库
```

各模块的边界如下：

- `routers` 是系统入口，负责把 HTTP 请求转成业务动作，但不直接执行长耗时研究任务。

- `schemas` 是接口契约或者其他数据模型，负责让输入输出保持结构化。

- `background` 是长任务入口，负责启动和执行大纲生成、报告生成、报告渲染等流程。

- `agents` 负责智能体逻辑，包括理解研究目标、生成大纲、协调信息检索和逐章节保存研究结果。

- `tools` 负责外部能力接入，所有工具都应该有清晰的输入和输出。

- `repository` 是数据库边界，路由、后台任务和 Agent 不直接拼数据库操作细节。

### 2.4 接口设计

定义接口，本质上是定义系统如何和外界交互。接口需要同时回答三个问题：

1. 前端需要提交什么数据？

2. 后端会返回什么结构？

3. 当前操作会改变哪些项目状态或任务状态？

本项目所包含的核心接口如下：

| 接口                                                      | 方法     | 用途                       |
| ------------------------------------------------------- | ------ | ------------------------ |
| `/health`                                               | `GET`  | 服务健康检查                   |
| `/api/v1/research-projects`                             | `POST` | 创建研究项目，并自动提交研究任务书和大纲生成任务 |
| `/api/v1/research-projects/{project_id}/outline`        | `GET`  | 获取大纲草案                   |
| `/api/v1/research-projects/{project_id}/outline`        | `PUT`  | 确认大纲或提交大纲修改意见            |
| `/api/v1/research-projects/{project_id}/report-tasks`   | `POST` | 提交报告生成任务                 |
| `/api/v1/tasks/{task_id}`                               | `GET`  | 查询后台任务状态                 |
| `/api/v1/research-projects/{project_id}/reports/latest` | `GET`  | 获取最新报告                   |

项目状态和任务状态是两个不同概念：

| 类型   | 示例                                                                                       | 含义                 |
| ---- | ---------------------------------------------------------------------------------------- | ------------------ |
| 项目状态 | `brief_generating`、`outline_ready`、`outline_confirmed`、`research_running`、`report_ready` | 表示一个研究项目当前走到哪个业务阶段 |
| 任务状态 | `queued`、`running`、`succeeded`、`failed`                                                  | 表示某个后台任务当前是否执行完成   |

#### 2.4.1 创建研究项目

##### 请求

```http
POST /api/v1/research-projects
```

```json
{
  "topic": "研究具身智能行业未来三年的机会",
  "research_goal": "判断公司是否需要关注该行业",
  "target_audience": "公司战略团队",
  "region_scope": "china",
  "time_scope": {
    "type": "recent_years",
    "years": 3
  }
}
```

字段说明：

- `topic`：研究主题。

- `research_goal`：研究目标，用于约束报告最终回答什么问题。

- `target_audience`：目标读者，用于影响报告的表达方式和信息粒度。

- `region_scope`：地域范围，支持 `china`、`overseas`、`global`。

- `time_scope.type`：时间范围类型，支持 `recent_years`、`unlimited`。

- `time_scope.years`：当 `time_scope.type` 为 `recent_years` 时使用，表示近 N 年。

##### 响应

```json
{
  "project_id": "项目编号",
  "initial_task_id": "任务编号",
  "initial_task_type": "generate_research_brief",
  "topic": "研究具身智能行业未来三年的机会",
  "status": "brief_generating",
  "created_at": "2026-06-05T08:00:00Z"
}
```

说明：

- 创建研究项目后，后端自动创建大纲生成任务。

- 前端不需要再调用单独的大纲生成接口。

- `project_id` 贯穿整个研究生命周期。

- `initial_task_id` 用于查询“研究任务书和大纲草案生成任务”的状态。

- 大纲生成完成后，项目状态变为 `outline_ready`。

#### 2.4.2. 获取大纲草案

##### 请求

```http
GET /api/v1/research-projects/{project_id}/outline
```

##### 响应

```json
{
  "project_id": "项目编号",
  "status": "outline_ready",
  "outline": [
    {
      "node_id": "1",
      "title": "行业定义和研究边界",
      "question": "本报告讨论的具身智能具体指什么",
      "description": "明确行业定义、研究对象和本报告不覆盖的边界",
      "children": [
        {
          "node_id": "1.1",
          "title": "行业定义",
          "question": "具身智能和传统机器人、通用人工智能有什么区别",
          "description": "说明核心概念、技术边界和典型应用形态",
          "children": []
        },
        {
          "node_id": "1.2",
          "title": "研究范围",
          "question": "本报告覆盖哪些地区、时间范围和应用场景",
          "description": "说明本次研究的地域范围、时间范围和场景边界",
          "children": []
        }
      ]
    },
    {
      "node_id": "2",
      "title": "市场规模和增长驱动",
      "question": "行业未来三年是否存在足够大的增长空间",
      "description": "分析市场规模、增速、核心驱动因素和不确定性",
      "children": []
    }
  ]
}
```

字段说明：

- `node_id`：大纲节点 ID，用于前端展开、定位和后续编辑。

- `title`：章节标题。

- `question`：该章节需要回答的核心问题。

- `description`：该章节的写作说明。

- `children`：子章节，支持多级嵌套。

#### 2.4.3. 保存已确认大纲

##### 请求

```http
PUT /api/v1/research-projects/{project_id}/outline
```

用户可以直接确认大纲，也可以用自然语言要求系统修改大纲。

直接确认大纲：

```json
{
  "action": "confirm"
}
```

要求系统修改大纲：

```json
{
  "action": "revise",
  "revision_instruction": "把竞争格局单独拆成一章，并增加头部公司的产品对比"
}
```

##### 响应

直接确认大纲时：

```json
{
  "project_id": "项目编号",
  "status": "outline_confirmed"
}
```

要求系统修改大纲时：

```json
{
  "project_id": "项目编号",
  "revision_task_id": "任务编号",
  "status": "outline_revising"
}
```

说明：

- `action=confirm` 表示用户不修改大纲，直接确认，项目状态变为 `outline_confirmed`。

- `action=revise` 表示用户用自然语言要求系统调整大纲，后端启动大纲修改任务，项目状态变为 `outline_revising`。

- 大纲修改完成后，项目状态重新变为 `outline_ready`。

- 前端继续调用 `GET /api/v1/research-projects/{project_id}/outline` 获取最新大纲。

#### 2.4.4. 提交报告生成任务

##### 请求

```http
POST /api/v1/research-projects/{project_id}/report-tasks
```

```json
{
  "user_instruction": "报告风格偏管理层汇报，结论要明确"
}
```

##### 响应

```json
{
  "task_id": "任务编号",
  "project_id": "项目编号",
  "task_type": "generate_report",
  "status": "queued"
}
```

说明：

- 只有项目状态为 `outline_confirmed` 时，才能提交报告生成任务。

- 提交成功后，项目状态变为 `research_running`。

- 报告生成任务内部包含研究执行和报告渲染两个阶段。

#### 2.4.5. 查询任务状态

##### 请求

```http
GET /api/v1/tasks/{task_id}
```

##### 响应

```json
{
  "task_id": "任务编号",
  "project_id": "项目编号",
  "task_type": "generate_report",
  "status": "running",
  "message": "正在检索公开资料",
  "created_at": "2026-06-05T08:00:00Z",
  "updated_at": "2026-06-05T08:03:00Z"
}
```

任务状态接口用于支持前端轮询。HTTP 请求不会一直阻塞等待后台研究完成，前端通过 `task_id` 查询任务进度。

#### 2.4.6. 获取最新报告

##### 请求

```http
GET /api/v1/research-projects/{project_id}/reports/latest
```

##### 响应

```json
{
  "project_id": "项目编号",
  "report_id": "报告编号",
  "version": 1,
  "title": "具身智能行业机会研究报告",
  "html": "<html>报告正文</html>",
  "sources": [
    {
      "source_id": "S1",
      "title": "来源标题",
      "url": "https://example.com/report",
      "published_at": "2026-01-01",
      "source_type": "public_web"
    }
  ],
  "created_at": "2026-06-05T08:30:00Z"
}
```

说明：

- `html` 是已经渲染好的报告正文。

- `sources` 是报告引用的来源列表。

- `version` 用于表示报告版本，后续可以支持重新渲染或多版本对比。

### 2.5 编码

基于前面的接口设计，骨架代码可以按下面的顺序构建：

1. 在 `schemas` 模块下定义接口出入参和状态枚举。

2. 在 `routers` 模块下定义接口路由函数。

3. 在 `config` 模块下集中管理系统配置。

4. 在 `main.py` 中创建 FastAPI 应用并注册路由。

第一阶段的代码重点是先把系统入口和接口契约搭起来。

路由函数可以分为两类：

- 写操作接口：例如创建项目、确认大纲、提交报告任务。这类接口会创建或更新项目状态，并启动后台任务。

- 读操作接口：例如查询大纲、查询任务状态、获取最新报告。这类接口只从数据库读取结果并返回。

第一阶段完成后，系统应该具备以下骨架：

```text
app
  config
    config.py
  schemas
    __init__.py
  routers
    __init__.py
  main.py
```

这一步完成后，后续就可以继续补充后台任务、数据库仓储、Agent 和工具模块。
