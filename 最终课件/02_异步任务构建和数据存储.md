...
在教学过程中，可以先用 `asyncio.create_task()` 理解“接口立即返回、后台继续执行”的基本思想。但当前项目代码已经使用 Celery + Redis 作为实际任务队列。

在当前项目中，路由层不会直接等待大纲生成或报告生成完成，而是创建任务记录后，调用后台任务启动函数：

```python
start_generate_research_brief_task(project_id=project_id, task_id=task.task_id)
```

早期进程内版本可以把后台任务模块封装成 `asyncio.create_task`：
...
当前项目已经完成了这一步升级：`start_xxx_task` 不再创建进程内协程，而是把任务投递给 Celery。
...
本项目课程中仍然会讲 `asyncio.create_task`，是因为它有助于学生理解异步任务的本质；但代码主线以 Celery 为准。
...
| 后台任务模块 | `app/background/research_tasks.py`           | 实现 `start_xxx_task` 任务投递和 `run_xxx_task` 任务执行流程                           |
...
这些数据存在大量嵌套结构，而不是传统关系型数据库中二维表的平铺结构。例如，一个研究项目下面会包含大纲树、确认后的大纲、逐章节研究结果和项目级来源列表。
...
如果用关系型数据库，需要拆成项目表、大纲表、章节表、来源表、报告表等多个表。第一版系统为了快速跑通主链路，使用 MongoDB 会更直接。
...
| 文件                               | 职责                                   |
| -------------------------------- | ------------------------------------ |
| `research_project_repository.py` | 保存项目、更新项目状态、保存大纲、保存确认大纲、保存章节、合并项目级来源 |
