...
| 阶段   | 执行者                 | 职责                    |
| ---- | ------------------- | --------------------- |
| 研究阶段 | 研究管理智能体             | 写章节正文、整理关键发现、风险说明和章节来源 |
| 检索阶段 | 信息检索智能体             | 搜索资料、读取网页、检索知识库、整理来源  |
| 渲染阶段 | `write_html_report` | 把临时组装的渲染输入转换为 HTML      |
...
flowchart TD
    ...
    研究Agent[研究管理智能体]
    检索Agent[信息检索智能体]
    章节结果[ResearchProject.sections]
    项目来源[ResearchProject.sources]
    研究结果[临时 ResearchResult]
...
研究结果是报告渲染的输入，但当前项目不把 `ResearchResult` 当成持久化字段保存。真实落库的是 `ResearchProject.sections` 和 `ResearchProject.sources`；渲染前由代码临时组装 `ResearchResult`，再交给确定性渲染器。
...
其中，`sections` 是报告正文的主要来源；`sources` 是引用来源列表；`fact_cards` 主要保留为检索阶段的中间材料和后续教学扩展点，当前渲染主链路不依赖它。
...
| `summary`        | 章节摘要                |
| `body`           | 完整章节正文              |
| `key_findings`   | 结构化关键发现，每条包含 `claim/source_ids/confidence` |
| `sources`        | 本章节引用的来源详情          |
| `tables`         | 可渲染的表格数据            |
| `charts`         | 可渲染或占位展示的图表数据       |
| `risks`          | 结构化不确定性、口径差异和风险说明   |
...
来源是网页、政策文件、行业报告或内部知识库片段。事实材料是从来源中提取出来的可复核陈述。章节关键发现是主研究智能体基于来源和事实材料形成的判断。
...
章节通过 `key_findings.source_ids` 和 `risks.source_ids` 指向本章 `sources` 中的来源：
...
async def write_html_report(
    ...
    **legacy_kwargs: Any,
    ...
    输入为代码根据已保存 sections/sources 临时组装的 research_result；输出为 title/html/sources。
...
| 展示引用关系  | 渲染 `key_findings.source_ids` 和 `risks.source_ids` |
| 展示表格    | 渲染 `tables`              |
...
这里的 HTML 样式和细节不是核心，真正重要的是边界：输入来自已保存的章节和来源，输出必须是稳定报告对象。
...
| 阶段   | 方法                         | 结果                             |
| ---- | -------------------------- | ------------------------------ |
| 研究执行 | `generate_research_result` | 逐章节调用 `save_research_section`，保存 `sections` 并合并 `sources` |
| 报告渲染 | `generate_report`          | 从项目里的 `sections/sources` 临时组装 `ResearchResult`，调用 `write_html_report` 生成 HTML |
...
| `report-render-tasks` | 否        | 已经有 `sections/sources`，只重新渲染 HTML |

独立渲染任务会先检查项目是否已经保存研究章节：
...
后台执行时，只读取已落库的 `sections/sources`，不重新触发检索和研究：
...
这个接口体现了研究和渲染的解耦：章节和来源一旦落库，就可以多次生成不同版本的展示报告。
...
| 后台任务已提交      | 确认 Celery 任务已成功投递到 Redis |
| 开始生成研究任务书和大纲 | 确认任务进入 running               |
| 研究章节已保存      | 确认 Agent 已通过 `save_research_section` 保存 sections |
...
| 报告渲染失败       | `sections` 缺失或结构不完整 | 检查项目文档中的 `sections` 和 `sources`          |
| 报告来源为空       | 研究阶段没有保存来源                 | 检查章节 `sources`、`key_findings.source_ids` 和 `risks.source_ids` |
...
| 异步任务     | 长耗时研究任务在后台执行              |
| 状态追踪     | 项目状态和任务状态可查询              |
| 数据沉淀     | MongoDB 保存项目、大纲、章节、来源和报告版本 |
| Agent 协作 | 研究管理智能体协调信息检索智能体          |
| 工具扩展     | 搜索、网页读取、RAGFlow、章节保存、报告渲染 |
| 报告生成     | 已保存章节和来源确定性渲染为 HTML        |
| 引用溯源     | 章节关键发现和风险说明可以追溯到来源           |
...
| 方向    | 扩展方式                                               |
| ----- | -------------------------------------------------- |
| 任务系统  | 增加更细粒度进度事件、worker 监控和失败重试策略 |
